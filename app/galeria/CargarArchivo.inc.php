...
if (empty($_FILES["file1"])) {
	...
	$carpeta = "../../assets/cover/galeria";//carpeta donde se suben los archivos
	
	$fileName = str_replace(' ', '-', $fileName);

	$directorio = DIRECTORIO_RAIZ."/assets/cover/galeria";//se crea la variable para la ruta de la carpeta
	...
	if(move_uploaded_file($fileTmpLoc, $carpeta."/".$nombreEmpresa.date("d-m-Y").$string_aleatorio.utf8_decode($fileName))){	
		...
		<img style="box-shadow: 0px 0px 10px 0px rgba(0,0,0,0.5);" src="<?php echo RUTA_GALERIA_COVER.$nombreEmpresa.date("d-m-Y").$string_aleatorio."$fileName";?>" class="imagen">
		<br><br>
