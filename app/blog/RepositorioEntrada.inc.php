...
class RepositorioEntradaBlog {
    ...
    public static function insertar_entrada($conexion, $entrada) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "INSERT INTO blog(autor_id, url, imagen, url_externa, titulo, texto, etiqueta, fecha, activa) VALUES(:autor_id, :url, :imagen, :url_externa, :titulo, :texto, :etiqueta, NOW(), :activa)";
    ...
    public static function obtener_cuatro_por_fecha_descendente($conexion) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM blog ORDER BY fecha DESC LIMIT 3";//aqui se modifica el numero de entradas que deben aparecer en una pagina especifica
    ...
    public static function obtener_todas_por_fecha_descendiente($conexion) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM blog ORDER BY fecha DESC LIMIT 40";
    ...
    public static function obtener_entrada_por_url($conexion, $url) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM blog WHERE url LIKE :url";
                $sentencia = $conexion -> prepare($sql);
                ...
                if (!empty($resultado)) {
                    $entrada = new EntradaBlog(
                            $resultado['id'], $resultado['autor_id'], $resultado['url'], $resultado['imagen'], $resultado['url_externa'],
    ...
    public static function obtener_entradas_al_azar($conexion, $limite) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM blog ORDER BY RAND() LIMIT $limite";
    ...
    public static function contar_entradas_activas_admin($conexion, $id_admin) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT COUNT(*) as total_entradas FROM blog WHERE autor_id = :autor_id AND activa = 1";
                $sentencia = $conexion -> prepare($sql);
    ...
    public static function contar_entradas_inactivas_admin($conexion, $id_admin) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT COUNT(*) as total_entradas FROM blog WHERE autor_id = :autor_id AND activa = 0";
                $sentencia = $conexion -> prepare($sql);
                $sentencia -> bindParam(':autor_id', $id_admin, PDO::PARAM_STR);
                $sentencia -> execute();
                $resultado = $sentencia -> fetch();
    ...
    public static function obtener_entradas_admin_fecha_descendente($conexion, $id_admin) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT a.id, a.autor_id, a.url, a.imagen, a.url_externa, a.titulo, a.texto, a.etiqueta, a.fecha, a.activa, COUNT(b.id) AS 'cantidad_comentarios' ";
                $sql .= "FROM blog a ";
    ...
    public static function titulo_existe($conexion, $titulo) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT * FROM blog WHERE titulo = :titulo";
                $sentencia = $conexion -> prepare($sql);
    ...
    public static function eliminar_comentarios_y_entrada($conexion, $entrada_id) {
        if (isset($conexion)) {
            try {
                ...
                $sql2 = "DELETE FROM blog WHERE id=:entrada_id";
    ...
    public static function actualizar_entrada($conexion, $id, $titulo, $url, $imagen, $url_externa, $texto, $etiqueta, $activa) {
        $actualizacion_correcta = false;

        if (isset($conexion)) {
            try {

                $sql = "UPDATE blog SET titulo = :titulo, url = :url, imagen = :imagen, url_externa = :url_externa, texto = :texto, etiqueta = :etiqueta, activa = :activa WHERE id = :id";
    ...
    public static function buscar_entradas_todos_los_campos($conexion, $termino_busqueda) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sql = "SELECT * FROM blog WHERE titulo LIKE :busqueda OR texto LIKE :busqueda OR etiqueta LIKE :busqueda ORDER BY fecha DESC LIMIT 25";
    ...
    public static function buscar_entradas_por_titulo($conexion, $termino_busqueda) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sql = "SELECT * FROM blog WHERE titulo LIKE :busqueda ORDER BY fecha DESC LIMIT 25";
    ...
    public static function buscar_entradas_por_contenido($conexion, $termino_busqueda) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sql = "SELECT * FROM blog WHERE texto LIKE :busqueda ORDER BY fecha DESC LIMIT 25";
    ...
    public static function buscar_entradas_por_autor($conexion, $termino_busqueda) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sql = "SELECT * FROM blog e JOIN administradores u ON u.id = e.autor_id WHERE u.nombre LIKE :busqueda ORDER BY e.fecha DESC LIMIT 25";
