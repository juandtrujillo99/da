...
include_once 'app/galeria/ValidadorEntradaEditada.inc.php';
...
if (isset($_POST['guardar_cambios_entrada'])) {
	...
	$validador = new ValidadorEntradaEditadaGaleria($_POST['url'], $_POST['url-original'], $_POST['imagen'], $_POST['imagen-original'], $_POST['texto'], htmlspecialchars($_POST['texto-original']), $entrada_publica_nueva, $_POST['publicar-original'], Conexion :: obtener_conexion());

	if (!$validador -> hay_cambios()) {
		Redireccion :: redirigir(RUTA_GESTOR_ENTRADAS_GALERIA);
	} else {
		if ($validador -> entrada_valida()) {
			$cambio_efectuado = RepositorioEntradaGaleria :: actualizar_entrada(Conexion :: obtener_conexion(),
				$_POST['id-entrada'], $validador -> obtener_url(), $validador -> obtener_imagen(),
						...
						<form method="post" action="<?php echo RUTA_EDITAR_ENTRADA_GALERIA ?>">
							<?php
								if (isset($_POST['editar_entrada'])) {
									...
									$entrada_recuperada = RepositorioEntradaGaleria :: obtener_entrada_por_id(
										Conexion::obtener_conexion(), $id_entrada);

									include_once 'seccion/galeria/form_entrada_recuperada.inc.php';
									...
									include_once 'seccion/galeria/form_entrada_recuperada_validada.inc.php';
								}
							?>				
						</form>
					</div>
